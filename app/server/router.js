...
module.exports = function(app) {
    app.get('/', function(req, res){
	    ...
	    Lectures.find({name:"Guest"}, function(err,docs) {
		var lecs = [];
		REC.setLecCode(docs,lecs);
		res.render('login', { message: 'New User:'+name, name: 'Guest',
...
console.log(req.query.Code);
		if(req.query.Code!='New') {
			...
			{ upsert:false,multi:true}, function(err){if(err){console.log(err);}});
		    var Classes = REC.newClasses(mongoose,req.query.Code,function(err){if(err){console.log(err);}});
...
console.log(req.body.Code);
		if(req.body.Code!='New') {
			...
			{ upsert:false,multi:true}, function(err){if(err){console.log(err);}});
		    var Classes = REC.newClasses(mongoose,req.body.Code,function(err){if(err){console.log(err);}});
